...
try:
    from models import KRS, Mahasiswa, MataKuliah, Nilai
except Exception:
    ...
    def _build_model_stub(name, fields):
        ...
        class _Stub:
            ...
            def __init__(self, *args):
                """Inisialisasi data model (stub)."""
                [setattr(self, k, v) for k, v in zip(fields, args)]

            def to_dict(self):
                """Serialize model (stub)."""
                return {k: getattr(self, k, None) for k in fields}

            @classmethod
            def from_dict(cls, data):
                """Deserialize model (stub)."""
                return cls(*[data.get(k) for k in fields])

            def __str__(self):
                """Representasi string model (stub)."""
                return f"{name}({', '.join(str(getattr(self, k, '')) for k in fields[:2])})"

        _Stub.__name__ = name
        return _Stub

    Mahasiswa = _build_model_stub("Mahasiswa", ["nim", "nama", "id_prodi", "angkatan"])
    MataKuliah = _build_model_stub(
        "MataKuliah", ["kode", "nama", "sks", "semester", "tipe"]
    )
    KRS = _build_model_stub("KRS", ["nim", "list_kode_mk"])
    Nilai = _build_model_stub(
        "Nilai", ["nim", "kode_mk", "tugas", "uts", "uas", "nilai_akhir", "grade"]
    )
try:
    from structures.hash_table import HashTable
    from structures.linked_list import CircularLL, DoubleLL, SingleLL
    from structures.stack_queue import Queue, Stack
    from structures.tree_graph import Graph, Tree
except Exception:
    ...
    class HashTable:
        ...
        def __init__(self):
            """Inisialisasi HashTable (stub)."""
            self._data = {}

        def insert(self, key, value):
            """Insert data ke HashTable (stub)."""
            self._data[key] = value

        def search(self, key):
            """Cari data di HashTable (stub)."""
    ...
    class SingleLL:
        ...
        def __init__(self):
            """Inisialisasi SingleLL (stub)."""
            self._items = []

        def append(self, data):
            """Append data ke SingleLL (stub)."""
            self._items.append(data)

        def display(self):
            """Display SingleLL (stub)."""
    ...
    class DoubleLL(SingleLL):
        ...
        def traverse_backward(self):
            """Traverse mundur DoubleLL (stub)."""
            return list(reversed(self._items))

    class CircularLL(SingleLL):
        """Stub Circular Linked List jika structures belum tersedia."""
    ...
    class Queue:
        ...
        def __init__(self):
            """Inisialisasi Queue (stub)."""
            self._items = []

        def enqueue(self, item):
            """Enqueue ke Queue (stub)."""
            self._items.append(item)

        def dequeue(self):
            """Dequeue dari Queue (stub)."""
            return self._items.pop(0) if self._items else None

        def is_empty(self):
            """Cek Queue kosong (stub)."""
    ...
    class Tree:
        """Stub Tree jika structures belum tersedia."""
    ...
    class Graph:
        ...
        def __init__(self):
            """Inisialisasi Graph (stub)."""
            self._adj = {}

        def add_vertex(self, vertex):
            """Tambah vertex Graph (stub)."""
            self._adj.setdefault(vertex, [])

        def add_edge(self, v1, v2):
            """Tambah edge Graph (stub)."""
            self._adj.setdefault(v1, []).append(v2)
            self._adj.setdefault(v2, [])

        def get_neighbors(self, vertex):
            """Ambil neighbors Graph (stub)."""
...
try:
    ...
    def bubble_sort(arr, key):
        """Stub bubble_sort jika algoritma.py belum tersedia."""
        return arr

    def insertion_sort(arr, key):
        """Stub insertion_sort jika algoritma.py belum tersedia."""
        return arr

    def linear_search(arr, keyword, field):
        """Stub linear_search jika algoritma.py belum tersedia."""
        return None

    def binary_search(arr, nim):
        """Stub binary_search jika algoritma.py belum tersedia."""
        return None

    def hitung_ipk(nilai_list, index=0, trace=False):
        """Stub hitung_ipk jika algoritma.py belum tersedia."""
        return 0.0
...
def _ensure_data_dir():
    """Pastikan folder data ada (tanpa DS khusus)."""
    os.makedirs(_DATA_DIR, exist_ok=True)


def _data_path(filename):
    """Bangun path file data secara relative (tanpa DS khusus)."""
    return os.path.join(_DATA_DIR, filename)


def _as_list(data):
    """Pastikan data berbentuk list (List)."""
    return data if isinstance(data, list) else []


def _as_dict(data):
    """Pastikan data berbentuk dict (Dict)."""
    return data if isinstance(data, dict) else {}


def _str(value):
    """Konversi nilai ke string aman (tanpa DS khusus)."""
    return str(value) if value is not None else ""


def _require_all(*values):
    """Validasi semua input wajib terisi (tanpa DS khusus)."""
    return all(values)
...
def tambah_mahasiswa(nim, nama, id_prodi, angkatan):
    ...
    nim_str = _str(nim)
    if any(
        isinstance(item, dict) and _str(item.get("nim")) == nim_str for item in data
    ):
        return False
    ...
    payload = (
        ...
        else {"nim": nim_str, "nama": nama, "id_prodi": id_prodi, "angkatan": angkatan}
    )
    data.append(payload)
...
def lihat_semua_mahasiswa():
    """Lihat semua mahasiswa dari file (List)."""
    return _as_list(load_data("mahasiswa.json"))


def jelajahi_mahasiswa():
    """Jelajahi mahasiswa via CircularLL (CircularLL)."""
    data = _as_list(load_data("mahasiswa.json"))
    if not data:
        return []
    linked = CircularLL()
    for item in data:
        try:
            ...
            return data
    displayed = linked.display() if hasattr(linked, "display") else None
    return displayed if displayed is not None else data
...
def cari_nama(keyword):
    """Cari mahasiswa by nama dengan linear search (List + linear_search)."""
    return (
        None
        if not keyword
        else linear_search(_as_list(load_data("mahasiswa.json")), keyword, "nama")
    )


def sort_mahasiswa(key):
    """Sort mahasiswa dengan bubble sort (List + bubble_sort)."""
    return bubble_sort(_as_list(load_data("mahasiswa.json")), key) if key else []
...
def tambah_matkul(kode, nama, sks, semester, tipe):
    ...
    kode_str = _str(kode)
    if any(
        isinstance(item, dict) and _str(item.get("kode")) == kode_str for item in data
    ):
        return False
...
def lihat_semua_matkul():
    """Lihat semua mata kuliah dari file (List)."""
    return _as_list(load_data("matakuliah.json"))
...
def tambah_node_tree(parent, child):
    """Tambah node ke Tree struktur akademik (Tree)."""
    if not _require_all(parent, child):
        return False
    try:
        result = _tree.insert(parent, child)
    except TypeError:
        result = _tree.insert(child)
    return True if result is None else bool(result)


def tampil_tree():
    """Tampilkan hierarki Tree struktur akademik (Tree)."""
    if not hasattr(_tree, "display"):
        return []
    data = _tree.display()
...
def input_nilai(nim, kode_mk, tugas, uts, uas):
    ...
    entity = Nilai(
        _str(nim), _str(kode_mk), tugas_val, uts_val, uas_val, nilai_akhir, grade
    )
    payload = (
        entity.to_dict()
        if hasattr(entity, "to_dict")
...
def hitung_ipk_mahasiswa(nim):
    ...
    if not nilai_list:
        return None
    akhir = [
        item.get("nilai_akhir", 0) for item in nilai_list if isinstance(item, dict)
    ]
    if not akhir:
        return None
...
def catat_log(aksi):
    """Catat aksi ke Stack log (Stack)."""
    return False if not aksi else (_log_stack.push(aksi) or True)


def tampil_log():
    """Tampilkan log aktivitas dari Stack (Stack)."""
